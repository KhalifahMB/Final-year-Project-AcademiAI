...
export default function QuizzesPage() {
  ...
  return (
    <AppShell
      ...
      actions={
        isStaff ? (
          <Button size="sm" asChild className="h-8 gap-1.5 px-3 text-xs shadow-sm">
          ...
          <Button
            ...
            onClick={() => gen.mutate()}
            disabled={generating}
            className="h-8 gap-1.5 bg-gradient-to-r from-indigo-500 to-violet-600 px-3 text-xs text-white shadow-sm hover:from-indigo-600 hover:to-violet-700"
          >
            {generating ? (
    ...
    >
      ...
      {generating && !isStaff && (
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-primary/30 bg-gradient-to-r from-indigo-500/5 to-violet-500/5 p-3.5">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 text-white">
    ...
    <li>
      <article
        className={cn(
          ...
          isPublished && 'hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[0_4px_16px_-8px_color-mix(in_oklab,var(--primary)_35%,transparent)]',
        )}
      >
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 flex-1 items-start gap-2.5">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500/10 to-violet-500/10 text-primary">
        ...
        <div className="mt-auto flex items-center justify-between pt-3.5">
          {isStaff ? (
            ...
            <Link
              ...
              className="inline-flex h-8 items-center gap-1 rounded-lg bg-primary px-3.5 text-xs font-medium text-primary-foreground shadow-sm transition-transform hover:scale-[1.02]"
            >
