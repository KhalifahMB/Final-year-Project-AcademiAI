    ...
    <AppShell title={quiz.data?.title || 'Take quiz'} description={quiz.data?.description}>
      ...
      {showStart && (
        <div className="mx-auto max-w-xl view-enter">
          <div className="rounded-2xl border bg-gradient-to-br from-indigo-50 to-violet-50 p-8 text-center shadow-sm dark:from-indigo-950/30 dark:to-violet-950/30">
            <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-white/70 text-primary shadow-sm dark:bg-white/10">
            ...
            <div className="mt-5 flex items-center justify-center gap-2">
              <Button
                ...
                className="h-9 gap-1.5 bg-gradient-to-r from-indigo-500 to-violet-600 px-5 text-xs text-white shadow-sm hover:from-indigo-600 hover:to-violet-700"
              >
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-5 lg:flex-row">
      ...
      <div id="quiz-question-area" className="min-w-0 flex-1 overflow-y-auto">
        ...
        <div className="mb-4">
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
            <div
              className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-600 transition-all duration-200"
        ...
        <div key={q.id} className="rounded-2xl border bg-card p-6 shadow-sm animate-slide-up">
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          {currentIdx < qs.length - 1 ? (
            ...
            <Button
              ...
              onClick={onSubmit}
              disabled={submitting}
              className="h-8 gap-1.5 bg-gradient-to-r from-emerald-500 to-teal-600 px-4 text-xs text-white shadow-sm hover:from-emerald-600 hover:to-teal-700"
            >
      ...
      <aside className="w-full shrink-0 rounded-xl border bg-card p-4 lg:w-60">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isFl && !answered && 'border-amber-400/50 bg-amber-500/5 text-amber-600',
                )}
                title={`Question ${i + 1}${answered ? ' · answered' : ''}${isFl ? ' · flagged' : ''}`}
              >
                {i + 1}
                {isFl && <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-amber-500" aria-hidden />}
              </button>
            );
          })}
        </div>
        <div className="mt-4 space-y-1.5 text-[10.5px] text-muted-foreground">
          ...
          <div className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-sm bg-amber-500" /> Flagged
          </div>
        </div>
        <Button
          type="button"
...
function QuizResults({ result, _qs, reviewIdx, setReviewIdx, retake, canRetake, onBack }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-5">
      {/* Summary hero */}
      <div className="overflow-hidden rounded-2xl border bg-gradient-to-br from-card to-accent/20 p-6 shadow-sm sm:p-8">
        <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-start sm:text-left">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              {canRetake && (
                <Button size="sm" onClick={retake} className="h-8 gap-1.5 bg-gradient-to-r from-indigo-500 to-violet-600 px-4 text-xs text-white shadow-sm hover:from-indigo-600 hover:to-violet-700">
      ...
      {review.length > 0 && current && (
        <div className="space-y-4">
          ...
          <div key={reviewIdx} className={cn(
            'rounded-2xl border-l-4 border bg-card p-5 shadow-sm animate-slide-up',
            current.is_correct ? 'border-l-emerald-500' : 'border-l-red-500',
          )}>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isCorrect && 'border-emerald-500/40 bg-emerald-500/8 text-emerald-700 dark:text-emerald-400',
                        isUser && !isCorrect && 'border-red-500/40 bg-red-500/8 text-red-700 dark:text-red-400',
              ...
              {review.map((r, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === reviewIdx ? 'scale-125 bg-primary' : r.is_correct ? 'bg-emerald-500/60' : 'bg-red-500/60',
                  )}
