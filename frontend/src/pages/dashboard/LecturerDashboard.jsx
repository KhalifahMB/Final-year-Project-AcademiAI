...
function Pipeline({ pipeline }) {
  ...
  return (
    <section className="card p-5">
      ...
      <div className="mt-3 flex h-1.5 overflow-hidden rounded-full bg-[var(--hover)]">
        {items.map((it) => {
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--surface-2)]/60 text-[11px] font-[600] uppercase tracking-[0.08em] text-[var(--muted)]">
                <th className="py-2.5 pl-4 pr-2 text-left">Student</th>
                <th className="px-2 text-right">Avg score</th>
                <th className="px-2 text-right">Attempts</th>
                <th className="py-2.5 pl-2 pr-4 text-right">Status</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((s) => (
      ...
      {items.length === 0 ? (
        ...
        <ul className="-mx-2 space-y-1">
          {items.slice(0, 5).map((a, i) => (
            <li key={i} className="rounded-[var(--radius-md)] px-2 py-2 transition-colors hover:bg-[var(--hover)]">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="eyebrow flex items-center gap-1.5">
            <HardHat className="h-3 w-3 text-[var(--accent-strong)]" aria-hidden />
            {workspace}
          </p>
