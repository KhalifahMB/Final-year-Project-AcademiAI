...
import {
  BookOpen, GraduationCap, LayoutDashboard, MessageSquare, FileText,
  ClipboardList, StickyNote, Bookmark, TrendingUp, User, Shield, Upload, Building2, Users,
} from "lucide-react";
...
export default function AppShell({ title, children }) {
  ...
  if (user?.role === "lecturer" || user?.role === "admin") items = [...items, ...lecturerExtra];
    ...
    <div className="min-h-screen bg-muted/40 flex flex-col">
      <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-4">
          <Link to="/dashboard" className="font-bold text-primary shrink-0">AcademiAI</Link>
          <nav className="hidden lg:flex flex-1 gap-0.5 overflow-x-auto" aria-label="Main">
            {items.map((n) => {
              const Icon = n.icon;
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="hidden sm:inline text-xs text-muted-foreground truncate max-w-[140px]">{user?.email} · {user?.role}</span>
