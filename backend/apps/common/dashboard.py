...
def _tenant_cache_key(user, suffix: str) -> str:
    ...
    # Dashboards are user-specific, so the key must include the user id to
    # avoid one user seeing another user's cached data.
    return f"dashboard:{tid}:{user.role}:{getattr(user, 'id', 'anon')}:{suffix}:v1"
...
@extend_schema(
    tags=["Dashboard"],
    summary="AI-generated personalized greeting",
    description="Returns a personalized greeting message generated by AI based on the user's recent activity.",
)
class AiGreetingView(APIView):
    ...
    def get(self, request):
        ...
        user = request.user
        cache_key = f"ai-greeting:{user.tenant_id}:{user.id}:v1"
        cached = cache.get(cache_key)
        if cached is not None:
            return Response(cached)

        # Build activity context for the greeting
        ...
        try:
            ...
            if chat_count:
                activity_parts.append(f"{chat_count} chat sessions")
        except Exception:
            pass
        ...
        try:
            ...
            if attempt_count:
                activity_parts.append(f"{attempt_count} quiz attempts")
        except Exception:
            pass
        ...
        try:
            ...
            resource_count = Resource.objects.filter(
                tenant=user.tenant,
            ).filter(_authorized_resources_q(user)).count()
            if resource_count:
                activity_parts.append(f"{resource_count} available resources")
        except Exception:
            pass

        activity_summary = ", ".join(activity_parts) if activity_parts else "no recent activity yet"

        prompt = (
            f"Generate a short, warm, personalized welcome greeting for {first_name}, "
            f"who is a {role}. Their recent activity includes: {activity_summary}. "
            "Keep it to 2-3 sentences maximum. Be encouraging and mention something "
            "specific from their activity if available. Do not use generic time-of-day "
            "greetings like 'Good morning'. Instead, make it feel personal and academic."
        )
        ...
        if client is None:
            greeting_text = (
                ...
                f"You have {activity_summary} to catch up on."
            )
            result = {"greeting": greeting_text, "suggested_action": "Start a new chat with the AI tutor"}
        ...
        try:
            ...
            greeting_text = (resp.text or "").strip()
            if not greeting_text:
                greeting_text = f"Welcome back, {first_name}! Ready to dive in?"
        except Exception:
            greeting_text = f"Welcome back, {first_name}! Ready to continue your learning journey?"
...
@extend_schema(
    tags=["Dashboard"],
    summary="AI-generated dashboard insight",
    description="Returns a personalized AI insight based on the user's real data.",
)
class AiInsightView(APIView):
    ...
    def post(self, request):
        ...
        user = request.user
        dashboard_type = request.data.get("dashboard_type", "student")
        # Only allow known dashboard types in the prompt — reject/fall back on
        # anything else to prevent prompt injection via this field.
        if dashboard_type not in ("student", "lecturer", "admin"):
            dashboard_type = "student"
        cache_key = f"ai-insight:{user.tenant_id}:{user.id}:{dashboard_type}:v1"
        cached = cache.get(cache_key)
        if cached is not None:
            return Response(cached)

        # Gather real data for the insight
        context = self._gather_context(user, dashboard_type)

        prompt = (
            f"You are an AI academic insights engine. Based on the following real data "
            f"for a {dashboard_type} user, generate a brief, actionable insight.\n\n"
            f"Data:\n{context}\n\n"
            "Return a JSON object with exactly these fields:\n"
            '- "headline": A single compelling sentence (max 15 words)\n'
            '- "body": 2-3 sentences of explanation (max 100 words)\n'
            '- "suggested_actions": Array of 1-3 specific action strings\n'
            '- "confidence": A number between 0.5 and 1.0\n\n'
            "Rules:\n"
            "1. Be specific — reference actual numbers and names from the data.\n"
            "2. Be encouraging but honest. If performance is low, suggest improvement steps.\n"
            "3. Never make up data. Only reference what's in the context.\n"
            "4. Return ONLY valid JSON, no markdown fences."
        )

        client = _get_client()
        if client is None:
            result = self._fallback_insight(user, dashboard_type, context)
        ...
        try:
            ...
            if isinstance(parsed, dict) and "headline" in parsed:
                result = {
                    ...
                    "suggested_actions": [str(a) for a in (parsed.get("suggested_actions") or [])[:3]],
                    "confidence": min(1.0, max(0.5, float(parsed.get("confidence", 0.7)))),
                }
            else:
                result = self._fallback_insight(user, dashboard_type, context)
        except Exception:
    ...
    @staticmethod
    def _gather_context(user, dashboard_type):
        """Build a summary of the user's real data for the AI."""
        parts = []
        try:
            from apps.learning.models import ProgressRecord
            records = ProgressRecord.objects.filter(user=user).select_related("concept")
            if records:
                avg = sum(r.progress_value for r in records) / len(records)
                parts.append(f"Concepts tracked: {len(records)}, Average mastery: {avg:.0f}%")
                weak = [r.concept.canonical_name for r in records if r.progress_value < 50][:3]
                if weak:
        ...
        try:
            from apps.chat.models import ChatSession
            chat_count = ChatSession.objects.filter(tenant=user.tenant, user=user).count()
            parts.append(f"Total chat sessions: {chat_count}")
        except Exception:
            pass

        try:
            from apps.assessments.models import QuizAttempt
            attempts = QuizAttempt.objects.filter(tenant=user.tenant, student=user)
            if attempts.exists():
                from django.db.models import Avg
                avg_score = attempts.aggregate(avg=Avg("score"))["avg"] or 0
                parts.append(f"Quiz average: {avg_score:.0f}%, Attempts: {attempts.count()}")
        except Exception:
            pass
        ...
        try:
            ...
            resource_count = Resource.objects.filter(
                tenant=user.tenant, processing_status="ready",
            ).filter(_authorized_resources_q(user)).count()
            parts.append(f"Available resources: {resource_count}")
        except Exception:
            pass

        try:
            from apps.learning.models import Plan
            active_plans = Plan.objects.filter(user=user, status="active").count()
            if active_plans:
                parts.append(f"Active plans: {active_plans}")
        except Exception:
            pass
    ...
    @staticmethod
    def _fallback_insight(user, dashboard_type, context):
        """Static fallback when AI is unavailable."""
        first_name = user.first_name or "there"
        return {
            "headline": f"Welcome back, {first_name}! Here's your learning overview.",
            "body": "Keep engaging with your materials and quizzes to unlock personalized AI insights.",
            "suggested_actions": ["Start a study plan", "Take a practice quiz", "Chat with the AI tutor"],
            "confidence": 0.5,
        }
