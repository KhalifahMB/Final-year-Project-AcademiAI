"""
Chat persistence helpers: sessions + messages (pure ORM, no HTTP concerns).
...
from apps.common.constants import CHAT_TITLE_MAX_CHARS
...
def bump_session_title(session, content: str) -> None:
    """Promote the default placeholder title to the user's first message."""
    if session.title in ("", "New chat"):
        session.title = content[:CHAT_TITLE_MAX_CHARS]
